...
Protected pre-read checks must establish disclosure safety for this known unsafe legacy taxonomy before its titles
or routes enter the model. If it cannot pass that check, use separately registered source search. Absent automatic
inheritance alone does not classify an ordinary provider-authorized taxonomy as unsafe. Removing restricted tokens
after loading them does not pass. New taxonomy publication follows the applicable access model in the
[publication and recovery acceptance](test-wiki-publication-recovery.md) contract.
