...
native sizes/ranges and then refined from observed evidence. At most ten verification read calls per operation,
...
do not reinterpret them as distinct-source limits. Unknown size uses conservative bounded reads and leaves room
...
- [ ] Run applicable adapter/contract and benchmark unit suites, skill validation, changed-link/whitespace checks,
...
interpretation evaluation is running from raw inputs and current skill files only. No provider operations ran.
