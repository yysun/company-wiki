...
[2026 年 9 月 12 日的试测](tests/rag-quality/pilot-2026-09-12.md)使用 `gpt-6-astra`，推理强度为 high：
...
提供方权限和完整生命周期仍须分别验收。新增的 7 项评分测试与现有的 23 项适配器/契约测试共 30 项，均已通过。

原始运行记录保存在 Git 忽略的本地 `tests/rag-quality/results/` 目录；已纳入版本管理的试测摘要保留了指标与适用范围。
基准测试指南说明了如何重新运行并评估新题目。
