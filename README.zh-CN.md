...
技能每个会话自动检查一次已记录仓库中的较新稳定版本。已是最新时不提示，检查不可用也不阻断 Wiki 工作。
发现更新后，先展示版本变化、相关更新说明、兼容性影响和安装目标；获得批准或已有明确升级请求后，
在两次 Wiki 操作之间安装，并保留本地修改、注册表配置和 Wiki 文档。详见[技能更新](skills/company-wiki/references/skill-updates.md)。
...
已有总读取预算替代未单独配置的来源/证据/复核默认上限；例如总计 20 次不会被新默认值额外压缩为 5 份来源或 10 次证据读取。
