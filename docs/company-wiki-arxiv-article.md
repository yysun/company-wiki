...
7. If evidence is insufficient and budget remains, refine source retrieval.
...
Add Source is deliberate reconciliation. Its compatibility alias, Ingest, does not denote an ETL pipeline or a background indexing stage. A title, keyword, or filename pattern can authorize bounded candidate discovery within the registered source scope. The agent reports a finite list of authorized exact targets and any coverage limits, then obtains user selection before reading their bodies for reconciliation, even for a single hit. A pattern or folder never silently selects every match, and discovery does not reset the read budget. Exact selected documents or an explicitly bounded batch support the smallest material proposal. Reconciliation with no material delta creates no processing receipt, duplicate node, or timestamp update. Original sources remain unchanged across all workflows.
...
Durable changes follow a separate protocol. A concrete proposal identifies targets, changes, evidence versions, destination scope, audience checks, applicable access model, write order, and operation semantics. Approval binds that particular intent. Immediately before applying it, the agent rechecks identity, authorization, evidence, targets, versions, and any explicitly required continuous inheritance. Material drift invalidates the approved plan.
...
This revision describes version 1.1.0 and subsequent fixes through repository commit `bc896b79edd7c55dfedf6f76d66e1b3d18a859b2`, inspected on September 14, 2026. The primary product sources are the [changelog](../CHANGELOG.md), [README](../README.md), [PRD v0.5](company-wiki_PRD_v0.5.md), [skill package](../skills/company-wiki/SKILL.md), and its [document format](../skills/company-wiki/references/document-format.md), [Query protocol](../skills/company-wiki/references/query.md), [Curate workflow](../skills/company-wiki/references/curate.md), [Add Source workflow](../skills/company-wiki/references/add-source.md), [publication boundary](../skills/company-wiki/references/publication.md), and [change protocol](../skills/company-wiki/references/change-protocol.md). Where planning documents are broader or older, the current focused workflow references define the operation described here.
